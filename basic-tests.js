...
describe("Basic Tests", () => {
  ...
  context('Signing', function () {
    ...
    it('can sign the transaction from the serialization reference in verbose mode', async function () {
      ...
      expect(prompts[0]).to.deep.equal([{"3":"Sign","17":"Transaction"}]);
      expect(prompts[1]).to.deep.equal([{"3":"Amount","17":"12345"}]);
      expect(prompts[2]).to.deep.equal([{"3":"To Address","17":"denali12yp9cc0melq83a5nxnurf0nd6fk4t224dtg0lx"}]);
      expect(prompts[3]).to.deep.equal([{"3":"To Address","17":"denali1cv6yz28qvqfgah34yw3y53su39p6kzzexk8ar3"}]);
      expect(prompts[4]).to.deep.equal([{"3":"Finalize","17":"Transaction"}]);
    });
